...
class PipelineConfig(BaseModel):
    ...
    artifact_path: Optional[Union[str, Path]] = Path("./nl2reasoning-artifacts")
...
class NL2ReasoningAutoConfig(PipelineConfig, BaseModel):
    num_domains: int = 10
    num_topics_per_domain: int = 10
    num_complexity_levels: int = 4


class NL2CReasoningManualConfig(PipelineConfig, BaseModel):
    domain_and_topics: dict[str, list[str]]
    complexity_levels: list[str]


ConfigLike = Union[NL2ReasoningAutoConfig, NL2CReasoningManualConfig, dict, str, Path]


def smart_load_pipeline_config(config: ConfigLike) -> PipelineConfig:
    if not isinstance(config, (NL2CReasoningManualConfig, NL2ReasoningAutoConfig)):
        config = smart_load_yaml(config)
        config = (
            NL2ReasoningAutoConfig(**config)
            if "num_domains" in config
            else NL2CReasoningManualConfig(**config)
        )
    return config
