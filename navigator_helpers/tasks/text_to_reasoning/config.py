...
class NL2ReasoningManualConfig(PipelineConfig, BaseModel):
    domain_and_topics: dict[str, list[str]]
    complexity_levels: list[str]


ConfigLike = Union[NL2ReasoningAutoConfig, NL2ReasoningManualConfig, dict, str, Path]


def smart_load_pipeline_config(config: ConfigLike) -> PipelineConfig:
    if not isinstance(config, (NL2ReasoningManualConfig, NL2ReasoningAutoConfig)):
        config = smart_load_yaml(config)
        config = (
            NL2ReasoningAutoConfig(**config)
            if "num_domains" in config
            else NL2ReasoningManualConfig(**config)
        )
    return config
from pathlib import Path
from typing import Optional, Union
