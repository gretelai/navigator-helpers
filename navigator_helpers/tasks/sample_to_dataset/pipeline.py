...
class Sample2DataSetPipeline:
    ...
    def __init__(
        self, config: SampleToDatasetConfig, llm_registry: LLMRegistry
    ) -> None:
        """
        ...
        self.tasks = SampleToDatasetTaskSuite(
            config=config, llm_registry=llm_registry, logger=logger
        )
        self._setup_logging()
    ...
    def run(
        ...
        logger.info(f"🧭 Navigator request received.")
        logger.info(
        ...
        seed_names = self.tasks.crowdsource_data_seed_types(
        ...
        generated_seeds = self.tasks.generate_data_seeds(
            seed_names, system_prompt_type=system_prompt_type
        )
        logger.info(f"  |-- 👀 Peeking at the data seeds: {generated_seeds}")
        logger.info(f"🔢 Creating data seed permutations")
