...
class SampleToDatasetTaskSuite:
    ...
    def _setup_agents(self) -> Dict[str, autogen.AssistantAgent]:
        ...
        agents = {}
        for prompt_type, system_prompt in [("reflection", REFLECTION_SYSTEM_PROMPT),
    ...
    def execute_prompt(self, user_prompt: str, prompt_type: str) -> Tuple[str, Union[dict, list]]:
        ...
        agent.reset()
        response = agent.generate_reply(messages=[{"content": f":{user_prompt}", "role": "user"}])
        ...
        if self.config.verbose:
            print(f'----------------------- Extracted JSON output ({prompt_type.upper()}) --------------------')
            print(json_output)
            print('------------------------------------------------------------------')
    ...
    def crowdsource_data_seeds(self, sample_dataset: pd.DataFrame, crowd_size=5, max_num_seeds=4, system_prompt_type='cognition') -> dict:
        ...
        data_seeds_list = []
        self.logger.info(
                f"  |-- Crowd size: {crowd_size}"
            )
        for i in range(crowd_size):
            self.logger.info(
                f"  |-- 🫡 assistant opinion {i+1}"
            )
            try:
                _, data_seeds = self.extract_data_seeds_in_one_shot(sample_dataset, system_prompt_type)
                ...
                is_valid_data_seeds, validation_result = validate_json_with_pydantic(ExampleDataSeedModel, data_seeds)
                if is_valid_data_seeds:
                    columns = data_seeds.get("columns", None)
                    data_seeds_list.append(columns)
                elif self.config.verbose:
                    print(f"Warning: data_seeds failed pydantic validation at crowd iteration {i+1}: {validation_result}")
            except Exception as e:
                if self.config.verbose:
                    print(f"Failed to extract data seeds at crowd iteration {i+1}: {e}")

        if not data_seeds_list:
            raise ValueError("All attempts returned None or missing 'columns', cannot proceed.")
        ...
        for seeds in data_seeds_list:
            if isinstance(seeds, list):  # Ensure schema is a list of columns
                ...
                print(f"Warning: Expected list, but got {type(seeds)} for a list of data seeds")

        self.logger.info(
            f"  |-- 🧐 examining, deduping and ranking seed types"
        )
        # Dedupe seeds
        seen_seeds = set()
        deduped_seeds = []
        ...
        for attempt in range(MAX_RETRIES):
            try:
                _, ranked_data_seeds = self.execute_prompt(dataseed_crowd_ranking_prompt, system_prompt_type)
                is_valid_ranked_data_seeds, validation_result = validate_json_with_pydantic(RankedDataSeedModel, ranked_data_seeds)

                if is_valid_ranked_data_seeds:
                    columns = ranked_data_seeds.get('columns', [])
                    sorted_columns = sorted(columns, key=lambda col: -int(col.get('quality_rank', 0)))
                    ...
                    if self.config.verbose:
                        ...
                        print("------------------- Generated Ranked Dataseeds  ------------")
                ...
                if attempt < MAX_RETRIES - 1 and self.config.verbose:
                    print(f"Ranking attempt {attempt + 1} failed: {str(e)}. Retrying ...")
                else:
                    print(f"Ranking failed after {MAX_RETRIES} attempts. Last error: {str(e)}")
                    # TODO: possibly fall back onto unranked seeds 
                    raise RuntimeError(f"Failed to rank data seeds after {MAX_RETRIES} attempts")
    ...
    def generate_data_seeds(self, dataseeds: dict, system_prompt_type='cognition') -> dict:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                _, generated_seeds = self.execute_prompt(data_seed_generation_prompt, system_prompt_type)
                
                if not isinstance(generated_seeds, dict) or 'columns' not in generated_seeds:
                    raise ValueError("Invalid response format: 'columns' not found in generated seeds")

                # Check if 'all_values' is present in all columns
                for column in generated_seeds['columns']:
                    if 'all_values' not in column:
                        raise ValueError(f"'all_values' not found in column: {column.get('name', 'unnamed')}")

                # Process the generated seeds
                for column in generated_seeds['columns']:
                    if isinstance(column['all_values'], str):
                        # If it's a string, split it
                        column['all_values'] = [value.strip() for value in column['all_values'].split(',')]
                    elif isinstance(column['all_values'], list) and len(column['all_values']) == 1 and isinstance(column['all_values'][0], str) and ',' in column['all_values'][0]:
                        # If it's a list with a single comma-separated string, split it
                        column['all_values'] = [value.strip() for value in column['all_values'][0].split(',')]
                    elif not isinstance(column['all_values'], list):
                        raise ValueError(f"Invalid 'all_values' format in column: {column.get('name', 'unnamed')}")
                ...
                if attempt < MAX_RETRIES - 1 and self.config.verbose:
                    ...
                    print(f"Failed to generate data seeds after {MAX_RETRIES} attempts: {str(e)}")
                    # TODO: possibly return example values as a fallback instead
                    raise RuntimeError(f"Failed to generate valid data seeds after {MAX_RETRIES} attempts")
    ...
    def generate_dataset_description(self, sample_dataset: pd.DataFrame, system_prompt_type='cognition') -> Union[dict, list]:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                _, dataset_description = self.execute_prompt(dataset_description_prompt, system_prompt_type)
                is_valid_dataset_description, validation_result = validate_json_with_pydantic(DatasetModel, dataset_description)
                ...
                if attempt < MAX_RETRIES - 1 and self.config.verbose:
                    print(f"Dataset description attempt {attempt + 1} failed: {str(e)}. Retrying ...")
                else:
                    print(f"Dataset description generation failed after {MAX_RETRIES} attempts. Last error: {str(e)}")
                    raise RuntimeError(f"Failed to rank data seeds after {MAX_RETRIES} attempts")
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_valid_data_generation_prompt:
                    ...
                    seed_emphasis = "Make sure to use the following context when generating data:\n" + \
                    "\n".join(f"{col['column_name'].replace('_', ' ')}: {{{col['column_name']}}}" for col in data_seed_descriptions['columns'])
                    data_generation_prompt["prompt"] += f"\n{seed_emphasis}"

                    # Include a few examples in JSONL format. Make sure to escape { and }
                    format_emphasis = "Here are a few examples of the data format:\n " + \
                    sample_dataset.sample(n=5).to_json(orient='records', lines=True).replace('{', '{{').replace('}', '}}')
                    data_generation_prompt["prompt"] += f"\n{format_emphasis}"

                    # Include data schema
                    data_schema = str(list(sample_dataset.columns))
                    schema_emphasis = f"The dataset should have only these columns: {data_schema}"
                    data_generation_prompt["prompt"] += f"\n{schema_emphasis}"
                    
                    if self.config.verbose:
                        print("------------------- Generated Data Generation Prompt  ------------")
                        pretty_print_json(data_generation_prompt)
                    return data_generation_prompt
                else:
                    raise ValueError(f"Invalid data generation prompt: {validation_result}")

            except Exception as e:
                if attempt < MAX_RETRIES - 1 and self.config.verbose:
                    print(f"Data generation prompt attempt {attempt + 1} failed: {str(e)}. Retrying ...")
                else:
                    print(f"Data generation prompt failed after {MAX_RETRIES} attempts. Last error: {str(e)}")
                    raise RuntimeError(f"Failed to produce a data generation prompt after {MAX_RETRIES} attempts")
    ...
    def generate_data(self, sample_dataset: pd.DataFrame, data_generation_prompt: dict, 
        ...
        def seed_data_prompt(row, template, seed_columns_list):
            ...
            for col in seed_columns_list:
                formatted_col = col.replace(' ', '_')
                formatted_template = formatted_template.replace(f"{{{col}}}", f"{{{formatted_col}}}")

            return formatted_template.format(**format_dict)

        def process_row(row, sample_dataset: pd.DataFrame, data_generation_prompt: dict, 
                        seed_columns_list: list, system_prompt_type: str='cognition') -> Optional[pd.DataFrame]:
            # format data prompt with a specific row of seeds
            data_prompt_w_seeds = seed_data_prompt(row, data_generation_prompt["prompt"], seed_columns_list)
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    _, data_jsonl = self.execute_prompt(jsonl_data_prompt_w_seeds, system_prompt_type)
                    ...
                    if set(df_synth.columns) != set(sample_dataset.columns):
                        mismatched_columns = set(df_synth.columns) ^ set(sample_dataset.columns)
                        raise ValueError(f"Column mismatch for id {row.id}. Mismatched columns: {mismatched_columns}")
                    ...
                    if attempt < MAX_RETRIES - 1 and self.config.verbose:
                        print(f"Attempt {attempt + 1} failed for id {row.id}: {str(e)}. Retrying...")
                    else:
                        if self.config.verbose:
                            print(f"Error processing data for id {row.id} after {MAX_RETRIES} attempts: {str(e)}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_row = {executor.submit(process_row, row, sample_dataset, data_generation_prompt, seed_columns_list, system_prompt_type): row
                            for row in seed_permutations.itertuples(index=False)}

            for future in tqdm(concurrent.futures.as_completed(future_to_row), total=len(future_to_row), desc="Generating data"):
                df = future.result()
                if df is not None:
        ...
        if generated_data_df:
            ...
            generated_data_w_seeds_df = pd.merge(seed_permutations, generated_data_df, on='id', how='inner')
